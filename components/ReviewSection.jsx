...
const ReviewSection = ({ productId }) => {
  ...
  const fetchReviews = useCallback(
    ...
    try{
      setLoading(true);
      const res = await axios.get(`/api/review?productId=${productId}&page=${pageNumber}&limit=${reviewsPerPage}`);
      if(pageNumber ===1){
...
const checkCanReview = useCallback(async()=>{
  ...
  try {
    const res = await axios.get(
      `/api/review/canReview?product=${productId}`
    );
...
const handleReviewSumbit = async(e)=>{
 ...
 const handleUpdateReview = async(e)=>{
  ...
  try{
    await axios.put("/api/review",{reviewId :  editingReview._id , ...userReview} );
    setUserReview({rating : 0 , comment : ""})
 ...
 const renderStars = (rating)=>{
  ...
  for(let i=1 ; i <=5; i++){
    if(i <= rating){
      ...
      starts.push(<StarHalf  className="w-5 h-5 text-yellow-400 fill-yellow-400" />)
    }else {
      ...
      <AnimatePresence>
        {(canReview || editingReview) && (
          <motion.form
            ...
            onSubmit={editingReview ? handleUpdateReview : handleReviewSumbit}
